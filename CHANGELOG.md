...
- **P3-1 CATiledLayer 超高分辨率分片渲染（原型）**：新增 `TiledImageView` 视图，
  以 CATiledLayer 为 backing layer，按需请求可见 tile、从源 CGImage 裁剪对应
  区域绘制（`CGImageCreateWithImageInRect` + ImageIO 懒解码），仅解码可见像素，
  避免 10000px+ 全分辨率图片首帧 GPU 同步解码卡顿；与主路径一致的零 EXIF
  变换与自适应几何。**默认关闭**（`TILED_RENDERING_ENABLED`），经真机对比
  首帧延迟/滚动流畅度/内存占用达标后可默认开启

### 🧪 测试与工程化

- 新增 tile 交集、显示几何、路由开关、回退容错 11 个测试
- 全量测试通过（1608 → 1619 passed）
...
- **P3-3 移除休眠模块**：删除零引用的 `ui/video_player_view.py`（`VideoPlayerView`，
  480 行，AVFoundation 视频播放组件）。项目已收敛为纯图片浏览，该组件从未被
  任何代码引用，存在两套未使用 UI 逻辑的维护成本；同步清理无关联测试与引用。
  文档 P3-3 提及的 `core/preload_manager.py` 已于 v2.4.1 清理中移除，本次核实
  无其他休眠模块残留
...
- **P3-4 图片元数据持久化缓存（`core/dimension_cache.py`）**：目录级"文件名 → 尺寸"
  映射以目录 mtime 为失效键持久化到应用支持目录，二次打开同一目录批量回填
  内存 LRU，跳过逐文件元数据读取；只读缓存 + 严格校验（损坏/格式非法安全忽略），
  LRU 上限 256 目录 + 原子写盘

### 🧪 测试与工程化

- 新增持久化缓存往返、mtime 失效、损坏容错、非法条目过滤、清空/单例 7 个测试
- 全量测试通过（1601 → 1608 passed）
...
- **P2-1 解码耗时自适应两阶段显示**：新增解码耗时经验表（按文件大小分档、
  近 5 次实测均值、LRU 上限 64 档 + 重置入口），每次后台解码后归档实测耗时；
  同一规格文件实测解码 ≥150ms 时自动启用"预览 → 懒解码全分辨率"两阶段，
  中低端机器上文件不大但解码慢（外置盘、高压缩 JPEG）时首帧更早可见，
  不牺牲全分辨率清晰度

### 🧪 测试与工程化

- 新增经验表分档/慢判定/样本不足防误判/重置/LRU 淘汰/两阶段触发 8 个测试
- 全量测试通过（1591 → 1601 passed）
...
- **P3-2 可重复性能基准（`scripts/benchmark.py`）**：合成多尺寸/格式图片集，
  度量首图加载延迟、连续翻页主线程阻塞（p50/p95/p99）、缓存命中率、
  RSS 内存曲线、文件夹冷/热扫描延迟；输出 JSON 基线（含版本/时间戳），
  支持 `--quick` 快速模式与 `--output` 文件输出，用于优化效果量化与防回归
...
- **P2-2 解码线程池有界队列**：关键池（当前图解码/next-ready）非关键后台任务
  （缓存升级、内存检查/进度保存）增加队列深度上限，快速导航时过期任务不再
  无限积压挤占关键解码线程；预取池上限收敛至 6
- **目录扫描复用（P2-3）**：`DirectoryImageListCache` 增加"目录是否含图"布尔缓存
  （mtime 失效），深度扫描阶段每目录只枚举一次；含图判断顺带填充图片列表缓存，
  消除"判断含图 → 加载列表"的重复枚举
- **渲染路径统一（P2-4）**：`drawRect_` CGImage 分支复用 `_get_image_display_rect`
  几何缓存与 `_get_transformed_rect`，删除内联重复计算
- **导航按键热路径减负**：防抖计算改用轻量入口 `calculate_optimal_debounce`，
  不再同步生成按键路径上不会被消费的预加载索引
- **热路径配置快照**：`full_res_browse` / `progressive_loading_enabled` 构造时
  快照到实例属性，翻页/显示路径不再重复 RLock 查询

### 🧪 测试与工程化

- 新增目录含图布尔缓存、有界提交丢弃语义等 6 个测试
- 修复 `unify_version.py` 对子目录测试文件的版本同步 bug
- 全量测试通过（1587 → 1591 passed）
