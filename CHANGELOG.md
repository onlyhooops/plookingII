...
- **性能跟踪报告改为会话级单文件**：一次运行（一次进程启动）只生成一份
  `perf_<启动时间戳>.json/.md` 报告，周期自动落盘与退出落盘合并覆盖同一
  文件，不再产生分散的 `_auto`/`_quit` 片段；报告含完整最终状态
  （操作统计 + 慢事件 + 内存曲线 + 会话元信息 + 最近更新原因），MD 标注
  "单次运行的完整会话记录"

### 🧪 测试与工程化

- 新增单会话单文件测试；轮转测试改为多会话场景验证
- README 同步更新性能跟踪说明
- 全量测试通过（1619 → 1620 passed）
