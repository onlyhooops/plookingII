...
1. `plookingII/core/history.py:67`
1. `plookingII/core/history.py:76`
1. `plookingII/core/network_cache.py:353`
1. `plookingII/core/network_cache.py:372`
...
1. `plookingII/services/recent.py:178`
...
1. SQL语句结构固定：`DELETE FROM recent_folders WHERE folder_path IN (?,?,?)`
1. 实际值通过第二个参数传递，由数据库驱动处理转义
1. 没有用户输入直接拼接到SQL字符串
