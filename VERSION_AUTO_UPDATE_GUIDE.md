...
1. ✅ **零维护成本** - 开发者只需关注代码和 commit message
1. ✅ **关于菜单自动更新** - 用户总是看到最新版本号

### 最佳实践

1. **使用规范的 commit message** - 这是触发自动版本升级的关键
1. **不要手动修改版本号** - 让 semantic-release 自动处理
1. **定期查看 CHANGELOG.md** - 了解版本更新历史
1. **验证版本一致性** - 运行 `make verify-version`
