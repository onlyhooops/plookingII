...
IMAGE_PROCESSING_CONFIG = {
    ...
    # 主线程同步快速加载阈值（MB）：仅小于该值的图片在主线程同步解码，
    # 超过则走后台异步解码，避免大图解码阻塞 UI 导航
    "fast_sync_threshold_mb": 12,
    ...
    # PNG 格式专属优化配置（v2.2.0+）
    # PNG 使用 DEFLATE 无损压缩，解码开销显著高于 JPEG DCT，
    # 且无内嵌缩略图/EXIF方向，需要差异化处理。
    "png_optimization": {
        "enabled": True,  # 启用 PNG 专属优化
        # 缩略图降采样因子：PNG 比 JPEG 使用更大的降采样 (4 vs 2)
        "thumbnail_subsample_factor": 4,
        # 策略阈值因子：PNG 文件更早进入 Quartz/mmap 路径
        # PNG 的 6MB ≈ JPEG 的 10MB 解码开销
        "threshold_factor": 0.6,
        # 检测 Alpha 通道避免不必要的 RGBA 渲染
        "detect_alpha": True,
        # 跳过元数据解析提升加载速度
        "skip_metadata": True,
        # 预加载窗口因子：PNG 解码更慢，缩窄预加载窗口
