...
class ConfigManager:
                ...
                if schema.env_var:
                    ...
                    if env_value is not None:
                        try:
                            ...
                            logger.warning("环境变量%s转换失败: %s", schema.env_var, e)
            ...
            if schema.env_var:
                ...
                if env_value is not None:
                    try:
                        ...
                        logger.debug("环境变量覆盖: %s = %s", key, converted_value)
                    except Exception as e:
                        logger.warning("环境变量%s转换失败: %s", schema.env_var, e)
    ...
    def _validate_config(self, key: str, value: Any) -> bool:
        ...
        try:
            ...
            logger.exception("配置值类型错误: %s = %s", key, value)
            return False
        ...
        if schema.validator:
            try:
                if not schema.validator(value):
                    logger.error("配置值验证失败: %s = %s", key, value)
                    return False
            except Exception as e:
