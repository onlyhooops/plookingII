...
class HybridImageProcessor:
    ...
    def __init__(self):
        ...
        # 智能缓存（预留上限常量，格式/阈值缓存当前仅定义未启用）
        self.format_performance_cache = {}  # 格式性能缓存（未启用）
        self.size_threshold_cache = {}  # 尺寸阈值缓存（未启用）
        self._MAX_FORMAT_CACHE = 128
        self._MAX_THRESHOLD_CACHE = 512
    ...
    def load_image_optimized(self, file_path, target_size=None, progressive=False, strategy="auto"):
        ...
        try:
            ...
            logger.exception("Failed to load image %s: %s", file_path, e)
            self._update_performance_stats(file_ext, time.time() - start_time)
            return None
    ...
    def _select_loading_strategy(self, strategy_name: str, file_path: str, file_size_mb: float):
        ...
        if strategy_name in self.loading_strategies:
            ...
            if strategy.can_handle(file_path, file_size_mb):
                return strategy
            logger.debug("Strategy %s cannot handle file size %sMB, falling back to auto", strategy_name, file_size_mb)
    ...
    def _execute_loading_strategy(self, strategy, file_path: str, target_size, file_size_mb: float):
        ...
        try:
            ...
            if not strategy.can_handle(file_path, file_size_mb, target_size):
                logger.warning("Strategy %s cannot handle %s", strategy.name, file_path)
                return None
            ...
            logger.exception("Strategy %s failed to load %s: %s", strategy.name, file_path, e)
            return None
