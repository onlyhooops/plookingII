...
class UnifiedMonitor:
    ...
    def __init__(
        ...
        # 内存状态短缓存（TTL）：导航热路径的 _compute_prefetch_window
        # 会频繁读取内存状态，psutil.virtual_memory() 是系统调用（~0.5-2ms），
        # 缓存可避免每次按键都做一次 sysctl 往返
        self._mem_status_cache: MemoryStatus | None = None
        self._mem_status_time: float = 0.0
        self._MEM_STATUS_TTL = 0.5  # 500ms

        # 启动自动监控
        if enable_auto_monitoring and HAS_PSUTIL:
            self.start_monitoring()

        logger.info("统一监控器已初始化 - 级别: %s, 历史: %s", level.value, self.max_history)
    ...
    def get_memory_status(self, force: bool = False) -> MemoryStatus:
        """获取当前内存状态

        Args:
            force: 为 True 时强制刷新（后台监控循环使用），否则优先返回
                   TTL 内的缓存值，避免热路径频繁系统调用。
        ...
        try:
            if not force:
                now = time.time()
                if self._mem_status_cache is not None and (now - self._mem_status_time) < self._MEM_STATUS_TTL:
                    return self._mem_status_cache
            ...
            # 更新短缓存
            self._mem_status_cache = status
            self._mem_status_time = time.time()
    ...
    def start_monitoring(self, interval: float = 5.0) -> None:
        ...
        def monitor_loop():
            while not self.stop_event.is_set():
                try:
                    # 获取内存状态
                    # 后台监控循环使用 force=True，确保决策基于最新数据而非 TTL 缓存
                    status = self.get_memory_status(force=True)

                    # 检查是否需要清理
                    if status.percent > self.memory_thresholds["cleanup"] * 100:
                        self.stats["memory_cleanups"] += 1
                        logger.warning("内存压力: %s (%.1f%%), 建议清理缓存", status.pressure_level, status.percent)
