        ...
        try:
            ...
            if hasattr(self.window, "subfolders") and hasattr(self.window, "current_subfolder_index"):
                save_data = {
                    ...
                    "keep_folder": getattr(self.window, "keep_folder", ""),
                    "current_folder": getattr(self.window, "current_folder", ""),
    ...
    def _async_save_progress(self) -> None:
        ...
        def save_worker():
            try:
                ...
                # 仅当待保存数据仍是本次快照时才清除：
                # 避免主线程在保存期间写入的更新数据被误清（竞态修复）
                with self._save_lock:
                    if self._pending_save_data is save_data:
                        self._pending_save_data = None
