...
class AppDiscovery:
    ...
    def __init__(self, workspace: NSWorkspace):
        ...
        self._cache_order: list[str] = []  # 按插入顺序跟踪缓存键，用于 LRU 淘汰
    ...
    def get_apps_for_file(self, file_path: str) -> list[AppInfo]:
        ...
        try:
            ...
            if not apps_urls:
                self._put_apps_cache(cache_key, [])
                return []
            ...
            self._put_apps_cache(cache_key, apps)
            return apps
    ...
    def _put_apps_cache(self, key: str, value: list[AppInfo]) -> None:
        """写入缓存，并执行 LRU 淘汰"""
        if key in self._cache_order:
            self._cache_order.remove(key)
        elif len(self._cache_order) >= self._MAX_APPS_CACHE_ENTRIES:
            oldest = self._cache_order.pop(0)
            self._apps_cache.pop(oldest, None)
        self._cache_order.append(key)
        self._apps_cache[key] = value
...
class MenuItemBuilder:
    ...
    def create_app_item(self, app_info: AppInfo) -> NSMenuItem:
        ...
        try:
            ...
            logger.debug("%s 图标设置失败: %s", app_info.name, e)
    ...
    def _get_app_icon(self, app_path: str):
        ...
        if len(self._icon_cache) >= self._MAX_ICON_CACHE:
            evict_count = max(1, self._MAX_ICON_CACHE // 2)
            keys_to_remove = list(self._icon_cache.keys())[:evict_count]
