...
class DragDropController:
    ...
    def setup(self):
        ...
        注册现代 + 旧版粘贴板类型以兼容所有 macOS 版本。
        """
        try:
            self.window.registerForDraggedTypes_([NSPasteboardTypeFileURL, NSFilenamesPboardType])
            logger.debug("拖拽接收功能已启用")
        except Exception as e:
            logger.warning("设置拖拽接收功能失败: %s", e)

    @staticmethod
    def _get_dragged_paths(pasteboard) -> list[str]:
        """从粘贴板获取被拖拽的文件路径

        macOS dragging pasteboard 上 NSFilenamesPboardType 始终可用（Finder 向后兼容），
        优先使用此路径。

        Args:
            pasteboard: NSPasteboard 对象

        Returns:
            文件路径字符串列表
        """
        # 优先：NSFilenamesPboardType（Finder 拖拽始终写入此类型）
        try:
            filenames = pasteboard.propertyListForType_(NSFilenamesPboardType)
            if filenames:
                logger.debug("Drag paths from NSFilenamesPboardType: %d items", len(filenames))
                return list(filenames)
        except Exception:
            pass

        # 回退：readObjectsForClasses（适用于非 Finder 来源）
        try:
            urls = pasteboard.readObjectsForClasses_options_([NSURL], {NSPasteboardURLReadingFileURLsOnlyKey: True})
            if urls:
                paths = [u.path() for u in urls if u.isFileURL()]
                if paths:
                    logger.debug("Drag paths from readObjectsForClasses: %d items", len(paths))
                    return paths
        except Exception:
            pass

        logger.debug("Drag paths: all methods returned empty")
        return []

    def prepare_for_drag_operation(self, sender) -> bool:
        """NSDraggingDestination: 放置前准备（始终允许，验证在 perform 中做）"""
        return True

    def concludeDragOperation_(self, sender):
        """NSDraggingDestination 协议方法：拖拽完成后的清理"""
    ...
    def dragging_entered(self, sender):
        ...
        try:
            ...
            filenames = self._get_dragged_paths(pasteboard)

            logger.info("拖拽进入: %d 个路径", len(filenames))
            if not filenames or len(filenames) == 0:
                logger.warning("拖拽进入: 未获取到文件路径，拒绝拖拽")
    ...
    def perform_drag_operation(self, sender):
        ...
        try:
            ...
            pasteboard = sender.draggingPasteboard()
            filenames = self._get_dragged_paths(pasteboard)
            logger.info("拖拽操作: 获得 %d 个路径: %s", len(filenames), filenames)

            if not filenames or len(filenames) == 0:
                logger.warning("拖拽操作: 未获取到任何文件路径")
                return False
            ...
            for filename in filenames:
                ...
                logger.info("拖拽路径检查: %s (isdir=%s)", filename, is_dir)
                if is_dir and self._folder_contains_images(filename):
                    valid_folder = filename
                    break
                if is_dir:
                    logger.info("文件夹不含支持的图片: %s", filename)

            if not valid_folder:
                logger.warning("拖拽操作: 未找到包含图片的有效文件夹")
                # 清除拖拽高亮效果
                self._clear_drag_highlight()

                # 使用友好的错误提示
                error = DragDropError("拖拽的文件夹中未找到支持的图片文件")
                show_error(error, "拖拽文件夹验证", self.window)
                return False
            ...
            if isinstance(e, DragDropError | FolderValidationError):
                show_error(e, "拖拽操作", self.window)
            else:
                error = DragDropError(f"拖拽操作失败: {e!s}")
                show_error(error, "拖拽操作", self.window)
