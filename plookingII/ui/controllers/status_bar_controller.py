...
class StatusBarController:
    ...
    def setup_ui(self, content_view, frame):
        ...
        # image_seq_label 与 folder_seq_label 共享同一个标签（兼容旧引用路径）
        self.image_seq_label = self.folder_seq_label
    ...
    def update_status_display(self, current_folder, images, current_index, subfolders, current_subfolder_index):
        ...
        try:
            if current_image_name and hasattr(self.main_window, "image_manager"):
                ...
                # 强转 str：缓存值可能未回填或类型异常，避免标题拼接时 TypeError
                resolution_text = str(getattr(img_mgr, "_current_image_resolution", "") or "")
                size_mb_text = str(getattr(img_mgr, "_current_image_size_mb", "") or "")
        except Exception:
            pass
