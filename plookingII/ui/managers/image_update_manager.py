...
class ImageUpdateManager:
    ...
    def set_current_image(self, image_path: str):
        ...
        try:
            ...
            logger.exception("设置当前监听图片失败: %s", e)
    ...
    def _on_file_changed(self, event: FileChangeEvent):
        ...
        try:
            ...
            logger.exception("处理文件变化事件失败: %s", e)
    ...
    def _handle_file_deleted(self, event: FileChangeEvent):
        ...
        def show_delete_notification():
            try:
                ...
                filename = os.path.basename(event.file_path)
                show_info("图片已删除", f'图片 "{filename}" 已被删除。', window=self.main_window)
    ...
    def _handle_file_moved(self, event: FileChangeEvent):
        ...
        Args:
            event: 文件变化事件
        """
        logger.info("检测到图片文件被移动: %s -> %s", event.old_path, event.file_path)
        ...
        def show_move_notification():
            try:
                ...
                show_info("图片已移动", f'图片已从 "{old_name}" 移动到 "{new_name}"。', window=self.main_window)

            except Exception as e:
                logger.exception("显示移动通知失败: %s", e)
    ...
    def reload_current_image(self):
        ...
        try:
            ...
            def show_reload_error():
                show_info("重新加载失败", f"重新加载图片时发生错误: {error_msg}", window=self.main_window)
    ...
    def _notify_update_callbacks(self, image_path: str, has_update: bool):
        ...
        for callback in self.update_callbacks:
            try:
                ...
                logger.exception("更新状态回调执行失败: %s", e)
    ...
    def cleanup(self):
        ...
        try:
            ...
            logger.exception("清理图片更新管理器失败: %s", e)
