...
from .alert_utils import present_alert_sheet, run_modal
...
class UserFeedbackManager:
    ...
    def show_error_dialog(self, error: Exception, context: str = "", window=None) -> None:
        """显示用户友好的错误对话框

        Args:
            error: 异常对象
            context: 错误上下文信息
            window: 宿主窗口；提供时优先以 sheet 展示
        """
        try:
            # 获取用户友好的错误信息
            ...
            def _on_end(response):
                # 如果用户点击帮助，显示详细帮助
                if response == 1001:  # 第二个按钮
                    self._show_help_dialog(error, window)

            # 优先 sheet，失败回退模态
            if not present_alert_sheet(alert, window, completion=_on_end):
                response = run_modal(alert, window)
                if response == 1001:  # 第二个按钮
                    self._show_help_dialog(error, window)
            ...
            # 回退到简单的错误提示
            self._show_simple_error(str(error), window)
        ...
        if isinstance(error, PlookingIIError):
            ...
            if error.severity == ErrorSeverity.CRITICAL:
                ...
                if error.severity == ErrorSeverity.HIGH:
                    return warning
                return info
        # 兼容不同 pyobjc/AppKit 版本：部分环境下 NSAlertStyle 不存在
        # 属性式常量，使用数字回退，避免抛异常导致弹窗流程中断
        return getattr(NSAlertStyle, "NSWarningAlertStyle", 1)

    def _show_help_dialog(self, error: Exception, window=None):
        """显示详细帮助对话框
        ...
        try:
            ...
            if not present_alert_sheet(alert, window):
                run_modal(alert, window)
    ...
    def _show_simple_error(self, message: str, window=None):
        """显示简单错误提示（回退方案）
    ...
    def show_info_message(self, title: str, message: str, window=None):
        """显示信息提示
        ...
        try:
            ...
            alert.setInformativeText_(message)
            alert.setAlertStyle_(getattr(NSAlertStyle, "NSInformationalAlertStyle", 0))
            alert.addButtonWithTitle_("确定")
            if not present_alert_sheet(alert, window):
                run_modal(alert, window)
        except Exception as e:
    ...
    def show_warning_message(self, title: str, message: str, window=None) -> bool:
        """显示警告提示
        ...
        try:
            ...
            alert.setMessageText_(title)
            alert.setInformativeText_(message)
            alert.setAlertStyle_(getattr(NSAlertStyle, "NSWarningAlertStyle", 1))
...
def show_error(error: Exception, context: str = "", window=None):
    """显示错误提示的便捷函数
    ...
    Args:
        ...
        context: 错误上下文
    """
    user_feedback_manager.show_error_dialog(error, context, window=window)


def show_info(title: str, message: str, window=None):
    """显示信息提示的便捷函数
    ...
    Args:
        ...
        message: 消息
    """
    user_feedback_manager.show_info_message(title, message, window=window)


def show_warning(title: str, message: str, window=None) -> bool:
    """显示警告提示的便捷函数
    ...
    Returns:
        bool: 用户确认结果
    """
    return user_feedback_manager.show_warning_message(title, message, window=window)
