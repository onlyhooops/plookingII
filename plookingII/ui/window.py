...
class MainWindow(NSWindow):
    ...
    def init(self):
        ...
        # 关键：将自己设为窗口代理，使 windowShouldClose_ 真正生效（关闭时隐藏而非销毁）
        # 并禁止窗口关闭时释放底层对象，保证 Dock 重新唤起与菜单栏目标始终有效
        try:
            self.setDelegate_(self)
            self.setReleasedWhenClosed_(False)
        except Exception:
            logger.debug("Failed to set window delegate/release behavior", exc_info=True)
    ...
    def _init_basic_attributes(self):
        ...
        # 状态栏更新合并（50ms 节流）：快速导航时避免每次按键
        # 都执行 setTitle_/setStringValue_ 等多次 AppKit 更新
        self._status_update_timer = None
        self._status_update_dirty = False
        self._STATUS_UPDATE_INTERVAL = 0.05  # 50ms ≈ 20Hz
    ...
    def windowShouldClose_(self, sender):
        ...
        而不是销毁窗口。仅清理会话级轻量资源（定时器），保留核心管理器
        （image_manager、navigation_controller）以供 Dock 恢复后继续使用。
        ...
        try:
            ...
            # 仅清理轻量级会话资源（保留核心管理器以供恢复）
            if hasattr(self, "navigation_controller") and self.navigation_controller:
                self.navigation_controller.cleanup()
            if hasattr(self, "image_update_manager") and self.image_update_manager:
                self.image_update_manager.cleanup()
    ...
    def _update_status_display_immediate(self):
        ...
        快速连按时多次调用合并为一次刷新，主线程 UI 工作量从
        “每键多次控件更新”降为“约 20Hz 一次”。
        """
        try:
            if self._status_update_timer is not None:
                # 已有待刷新的合并定时器，仅标记脏状态
                self._status_update_dirty = True
                return
            self._status_update_dirty = True
            self._status_update_timer = NSTimer.scheduledTimerWithTimeInterval_target_selector_userInfo_repeats_(
                self._STATUS_UPDATE_INTERVAL, self, "flushStatusUpdate:", None, False
            )
            try:
                from Foundation import NSRunLoop, NSRunLoopCommonModes

                NSRunLoop.currentRunLoop().addTimer_forMode_(self._status_update_timer, NSRunLoopCommonModes)
            except Exception:
                pass
        except Exception as e:
            logger.warning("更新状态栏显示失败: %s", e)

    def flushStatusUpdate_(self, timer):
        """合并定时器回调：一次性应用状态栏刷新"""
        self._status_update_timer = None
        try:
            if not self._status_update_dirty:
                return
            self._status_update_dirty = False
