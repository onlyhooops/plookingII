        ...
        def wrapper(*args, **kwargs):
            ...
            while attempt < max_retries:
                try:
                    ...
                    logger.warning("Retry %s/%s for %s due to: %s", attempt, max_retries, func.__name__, e)
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                if not check_func(*args, **kwargs):
                    logger.warning("%s for %s", error_msg, func.__name__)
                    return fallback
            except Exception:
                logger.exception("Boundary check error in %s", func.__name__)
                return fallback
