...
class TestPathUtils:
    ...
    def test_normalize_path_basic(self):
        ...
        result = path_utils.normalize_path_basic(path)
        assert result == path
        
        # 测试空路径
        result = path_utils.normalize_path_basic("")
        assert result == ""
        
        # 测试None
        result = path_utils.normalize_path_basic(None)
        assert result == ""
...
class TestErrorUtils:
    ...
    def test_safe_execute(self):
        ...
        result = ErrorUtils.safe_execute(success_func)
        ...
        result = ErrorUtils.safe_execute(error_func, default="default")
        assert result == "default"
    
    def test_log_error(self):
        """测试错误日志"""
        with patch('plookingII.utils.error_utils.logging') as mock_logging:
            error = ValueError("test error")
            ErrorUtils.log_error(error, "test context")
    ...
    def test_format_error_message(self):
        """测试错误消息格式化"""
        error = ValueError("test error")
        message = ErrorUtils.format_error_message(error, "test context")
    ...
    def test_handle_file_operation_error(self):
        ...
        result = ErrorUtils.handle_file_operation_error(error, "/test/path")
        
        assert result is False  # 默认返回False表示操作失败
    ...
    def test_is_recoverable_error(self):
        ...
        assert ErrorUtils.is_recoverable_error(recoverable_error) is True
        
        # 测试不可恢复错误
        fatal_error = SystemError("System error")
        assert ErrorUtils.is_recoverable_error(fatal_error) is False
...
class TestUtilsIntegration:
    ...
    def test_complete_file_processing_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            size = ErrorUtils.safe_execute(risky_operation, default=0)
            assert size > 0
