...
class TestMainFunction:
    ...
    def test_main_with_full_mock(self):
        """测试完全 mock 的 main 函数"""
        with patch('plookingII.app.main.NSApplication') as mock_ns_app, \
    ...
    def test_main_uses_error_context(self):
        ...
        with patch('plookingII.app.main.NSApplication') as mock_ns_app, \
            ...
            mock_error_context.assert_called_once_with(
                "app_main",
                category=mock_error_category.UI
            )
...
class TestAppDelegateLogic:
    ...
    def test_application_should_handle_reopen_logic(self):
        """测试重新打开应用的逻辑"""
        # 这个方法总是返回 True
        with patch.object(AppDelegate, 'applicationShouldHandleReopen_hasVisibleWindows_', 
    ...
    def test_application_should_terminate_logic(self):
        """测试应用终止的逻辑"""
        # 这个方法总是返回 True
        with patch.object(AppDelegate, 'applicationShouldTerminate_',
                         return_value=True) as mock_method:
