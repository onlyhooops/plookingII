...
class TestGlobalExceptionHook:
    ...
    def test_hook_logs_uncaught_exception(self, monkeypatch):
        """未捕获异常应进入统一错误处理器与 CRITICAL 日志"""
        import logging
        import sys

        import plookingII.core.error_handling as eh

        handle_error = MagicMock()
        monkeypatch.setattr(eh.global_error_handler, "handle_error", handle_error)
        monkeypatch.setattr(eh.logging, "critical", lambda *a, **k: None)
        monkeypatch.setattr(sys, "excepthook", lambda *a: None)

        install_global_exception_hook()
        err = ValueError("boom")
        eh._invoke_global_excepthook(err)
