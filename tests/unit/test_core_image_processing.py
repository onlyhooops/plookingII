...
def mock_image_processor_basic():
    """创建基本的图像处理器（禁用Quartz）"""
    with patch(
        "plookingII.core.image_processing.IMAGE_PROCESSING_CONFIG",
        {"quartz_enabled": False, "dual_thread_processing": False, "strict_quartz_only": False},
    ):
...
class TestHybridImageProcessorInit:
    ...
    def test_init_with_quartz_enabled(self):
        ...
        with patch(
            "plookingII.core.image_processing.IMAGE_PROCESSING_CONFIG",
            {"quartz_enabled": True, "dual_thread_processing": False, "strict_quartz_only": False},
        ):
            with patch("plookingII.core.image_processing.OptimizedLoadingStrategyFactory"):
                with patch("plookingII.core.image_processing.ImageRotationProcessor"):
                    processor = HybridImageProcessor()
    ...
    def test_init_strict_quartz_only_fails(self):
        ...
        with patch(
            "plookingII.core.image_processing.IMAGE_PROCESSING_CONFIG",
            {"quartz_enabled": False, "strict_quartz_only": True},
        ):
...
class TestStrategyExecution:
    ...
    def test_execute_loading_strategy_success(self, mock_image_processor_basic):
        ...
        result = mock_image_processor_basic._execute_loading_strategy(mock_strategy, "test.jpg", (800, 600), 5.0)
    ...
    def test_execute_loading_strategy_with_none_target(self, mock_image_processor_basic):
        ...
        result = mock_image_processor_basic._execute_loading_strategy(mock_strategy, "test.jpg", None, 5.0)
...
class TestEdgeCases:
    ...
    def test_very_large_file_size(self, mock_image_processor_basic):
        """测试非常大的文件"""
        # Mock os.stat 和 os.path.exists 以支持新的文件信息加载器
