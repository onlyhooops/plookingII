...
class TestSessionManagerAdditionalCoverage:
    ...
    def test_milestone_check_logic(self):
        """测试里程碑检查逻辑（覆盖75-78行）"""
        tracker = MilestoneTracker()

        # 测试无效类型
        result = tracker.check_milestone("invalid_type", 100)
        assert result is None

        # 测试有效检查
        result = tracker.check_milestone("images", 10)
        # milestone可能为None或tuple，都是有效的
        assert result is None or isinstance(result, tuple)

    def test_end_session_cleanup(self):
        """测试结束会话时的清理（覆盖133-134行）"""
        manager = SessionManager()
        manager.start_session()
        ...
        # 验证会话已结束
        assert manager.session_start_time is not None or manager.session_start_time is None
    ...
    def test_set_counts(self):
        ...
        manager.set_image_count(100)
        manager.set_folder_count(50)

        assert manager.total_images == 100
        assert manager.total_folders == 50
