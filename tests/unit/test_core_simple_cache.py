...
class TestSimpleImageCacheInit:
    ...
    def test_default_initialization(self):
        """测试默认初始化（v1.7.2 起默认 max_items=20, max_memory_mb=2000）"""
        cache = SimpleImageCache()

        assert cache.max_items == 20
        assert cache.max_memory_mb == 2000.0
...
class TestGlobalCache:
    ...
    def test_global_cache_default_config(self):
        """测试全局缓存默认配置（v1.7.2 起默认 max_items=20, max_memory_mb=2000）"""
        reset_global_cache()
...
class TestThreadSafety:
    ...
    def test_concurrent_puts(self):
        ...
        # 由于默认 max_items=20，最终应该有 20 个项（LRU 淘汰）
        assert len(cache) == 20
...
class TestEdgeCases:
    ...
    def test_none_value(self):
        """测试None值（NSCache 不接受 nil 值，固定走确定性实现）"""
        cache = SimpleImageCache()
        cache.put("key1", None)
