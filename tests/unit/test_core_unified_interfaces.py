...
class TestUnifiedCacheManager:
    ...
    def test_clear_cache_exception_handling(self):
        ...
        # 模拟内部异常
        with patch.object(manager, "_cache_providers", new_callable=dict):
            with patch("plookingII.core.unified_interfaces.logger"):
                # 强制触发异常
...
class TestUnifiedStatusManager:
    ...
    def test_set_status_message_exception_handling(self):
        ...
        # 模拟内部异常
        with patch.object(manager, "_status_providers", new_callable=dict):
            with patch("plookingII.core.unified_interfaces.logger"):
...
class TestEdgeCases:
    ...
    def test_clear_cache_specific_type_returns_false(self):
        ...
        provider = MockCacheProvider()
        # Mock clear_cache返回False
        provider.clear_cache = Mock(return_value=False)

        manager.register_cache_provider("test", provider)

        # 清理指定类型，provider返回False
        result = manager.clear_cache("test")

        # 应该返回False
        assert result is False
        # 但total_clears不应增加（因为success_count=0）
        assert manager._stats["total_clears"] == 0

    def test_clear_all_caches_some_return_false(self):
        """测试清理所有缓存时部分返回False（覆盖181->179分支）"""
        manager = UnifiedCacheManager()
        ...
        provider3 = MockCacheProvider()
        provider3.clear_cache = Mock(return_value=True)

        manager.register_cache_provider("success1", provider1)
        manager.register_cache_provider("fail", provider2)
        manager.register_cache_provider("success2", provider3)

        # 清理所有缓存
        result = manager.clear_cache()

        # 3个provider中只有2个成功，所以返回False
        assert result is False
        # total_clears应该增加2
        assert manager._stats["total_clears"] == 2
