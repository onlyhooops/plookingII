...
class TestFileUtilsEdgeCases:
    ...
    def test_special_characters_in_filename(self, temp_test_dir):
        """测试特殊字符文件名"""
        special_files = ["image (1).jpg", "image[2].jpg", "image{3}.jpg", "image@4.jpg"]
...
@pytest.mark.unit()
class TestFileUtilsExceptionHandling:
    """测试异常处理覆盖"""

    def test_folder_contains_images_with_subfolder_read_error(self, temp_test_dir):
        """测试子文件夹读取错误时的处理"""
        # 创建子文件夹并放入图片
        subfolder = temp_test_dir / "subdir"
        subfolder.mkdir()
        (subfolder / "image.jpg").touch()

        # Mock os.path.isdir to raise exception for subfolder
        with patch("os.path.isdir") as mock_isdir:
            mock_isdir.return_value = True
            with patch.object(FileUtils, "list_files_safe") as mock_list:
                # 第一次调用返回子文件夹列表，第二次抛出异常
                mock_list.side_effect = [["subdir"], Exception("Access denied")]

                # 应该捕获异常并返回False
                result = FileUtils.folder_contains_images(str(temp_test_dir), recursive_depth=1)
                # 由于主文件夹和子文件夹都没有图片（mock），应返回False
                assert result is False
    ...
    def test_get_image_files_recursive_with_exception_in_subfolder(self, temp_test_dir):
        ...
        subfolder = temp_test_dir / "subdir"
        subfolder.mkdir()
        (subfolder / "sub.jpg").touch()

        # Mock get_image_files在递归调用时抛出异常
        original_get = FileUtils.get_image_files
        call_count = [0]

        def mock_get_image_files(folder_path, recursive=False):
            call_count[0] += 1
            if call_count[0] > 1:  # 第二次调用（递归）时抛出异常
                raise Exception("Subfolder access error")
            return original_get(folder_path, recursive)

        with patch.object(FileUtils, "get_image_files", side_effect=mock_get_image_files):
            images = FileUtils.get_image_files(str(temp_test_dir), recursive=True)
            # 应该至少获取到主文件夹的图片
            assert len(images) >= 1
    ...
    def test_get_folder_info_with_isfile_exception(self, temp_test_dir):
        """测试获取文件夹信息时isfile抛出异常"""
        (temp_test_dir / "test.jpg").touch()

        with patch("os.path.isfile", side_effect=Exception("Access error")):
            total, images, has_sub = FileUtils.get_folder_info(str(temp_test_dir))
            # 异常被捕获，返回初始值
            assert total == 0
            assert images == 0
            assert has_sub is False

    def test_is_empty_folder_with_exception(self):
        """测试检查空文件夹时的异常处理"""
        with patch.object(FileUtils, "list_files_safe", side_effect=Exception("Error")):
            result = FileUtils.is_empty_folder("/some/path")
            # 异常时应返回True（视为空）
            assert result is True

    def test_folder_contains_images_list_safe_returns_none(self, temp_test_dir):
        """测试list_files_safe返回异常情况"""
        with patch.object(FileUtils, "list_files_safe", return_value=[]):
            result = FileUtils.folder_contains_images(str(temp_test_dir))
            assert result is False
