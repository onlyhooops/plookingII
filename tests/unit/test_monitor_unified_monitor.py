...
class TestGetMemoryStatusCache:
    """测试内存状态 TTL 缓存（P0-3）"""

    @pytest.mark.skipif(not HAS_PSUTIL, reason="需要 psutil")
    def test_memory_status_cached_within_ttl(self):
        """TTL 内重复调用返回缓存对象，避免重复系统调用"""
        monitor = UnifiedMonitor()
        first = monitor.get_memory_status()
        second = monitor.get_memory_status()
        assert first is second

    @pytest.mark.skipif(not HAS_PSUTIL, reason="需要 psutil")
    def test_force_refresh_bypasses_cache(self):
        """force=True 强制刷新，返回新对象"""
        monitor = UnifiedMonitor()
        first = monitor.get_memory_status()
        forced = monitor.get_memory_status(force=True)
