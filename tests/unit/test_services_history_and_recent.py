...
class TestProgressSaving:
    ...
    def test_save_task_progress_includes_folder_fields(self, history_manager):
        """待保存数据包含 keep_folder 与 current_folder"""
        history_manager.window.keep_folder = "/keep"
        history_manager.window.current_folder = "/photos/album"

        history_manager.save_task_progress()

        assert history_manager._pending_save_data["keep_folder"] == "/keep"
        assert history_manager._pending_save_data["current_folder"] == "/photos/album"
