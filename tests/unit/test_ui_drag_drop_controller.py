...
from plookingII.core.error_handling import DragDropError
...
class TestPerformDragOperation:
    ...
    @patch("plookingII.ui.controllers.drag_drop_controller.NSFilenamesPboardType")
    def test_perform_drag_operation_success(
        self, mock_pbtype, mock_basename, mock_isdir, drag_drop_controller, mock_drag_sender
    ):
        """测试成功执行拖拽"""
        mock_pbtype.return_value = "NSFilenamesPboardType"
    ...
    def test_perform_drag_operation_no_valid_folder(
        self, mock_show_error, mock_pbtype, mock_isdir, drag_drop_controller, mock_drag_sender
    ):
        """测试无有效文件夹"""
        mock_pbtype.return_value = "NSFilenamesPboardType"
        mock_isdir.return_value = True
    ...
    @patch("plookingII.ui.controllers.drag_drop_controller.NSFilenamesPboardType")
    @patch("plookingII.ui.controllers.drag_drop_controller.show_error")
    def test_perform_drag_operation_exception(
        self, mock_show_error, mock_pbtype, mock_isdir, drag_drop_controller, mock_drag_sender
    ):
        """测试操作异常"""
        mock_pbtype.return_value = "NSFilenamesPboardType"
...
class TestHelperMethods:
    ...
    def test_quick_folder_check_success(self, mock_get_loader, drag_drop_controller):
        ...
        mock_loader.scan_directory.return_value = [
            ...
            FileInfo(path="/test/folder/image2.png", extension="png", exists=True, is_file=True),
        ]
...
class TestIntegration:
    ...
    def test_complete_drag_workflow(self, drag_drop_controller, mock_drag_sender):
        ...
        with patch("plookingII.ui.controllers.drag_drop_controller.NSDragOperationCopy", 1):
            with patch("plookingII.ui.controllers.drag_drop_controller.os.path.isdir", return_value=True):
                with patch(
                    "plookingII.ui.controllers.drag_drop_controller.os.path.basename", return_value="test_folder"
                ):
                    test_folder = "/test/folder"
                    mock_drag_sender.draggingPasteboard().propertyListForType_.return_value = [test_folder]
